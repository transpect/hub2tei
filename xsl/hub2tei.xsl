...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="dbk:info[not(local-name(..) = ('book', 'figure'))]/dbk:title
                      |dbk:part/dbk:title
                      |dbk:chapter/dbk:title
                      |dbk:section/dbk:title" priority="4" mode="hub2tei:dbk2tei">
    <head type="main">
      <xsl:apply-templates select="@*, node()" mode="#current"/>
    </head>
  </xsl:template>
  
  <xsl:template match="dbk:bibliolist | 
                       dbk:bibliodiv[every $entry in * satisfies $entry[self::dbk:bibliomixed | self::dbk:biblioentry | self::dbk:title | self::dbk:info | self::dbk:bibliolist]]" mode="hub2tei:dbk2tei" priority="3">
    <listBibl>
      <xsl:apply-templates select="@*, node()" mode="#current"/>
    </listBibl>
  </xsl:template>
...
<!--  <xsl:template match="dbk:programlisting" mode="hub2tei:dbk2tei" priority="2">
  ...
  <xsl:template match="dbk:para[matches(@role, $tei:box-head1-role-regex) or matches(@role, $tei:box-head2-role-regex)]
                               [..[self::dbk:div | self::dbk:sidebar  | self::blockquote  | self::dbk:entry]]
